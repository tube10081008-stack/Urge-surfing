...
> ℹ️ **대화 모드(양방향)**: 화면이 위/아래 두 칸으로 나뉜다. 위 칸은 180°
> 회전되어 맞은편 상대가 똑바로 본다. 아래(나)는 내 언어로, 위(상대)는 상대
> 언어로 말하면 각각 반대편 언어 음성으로 통역된다. 한 칸을 탭하면 그 방향으로
> 통역 시작, 다시 탭하면 정지. 방향 전환 시 마이크는 유지하고 세션만 빠르게
> 재연결한다.
>
